...
class frequency_loss(torch.nn.Module):
    ...
    def forward(self, outputs, batch_y):
        if outputs.is_complex():
            ...
            frequency_outputs = self.fft(outputs.float(), dim=1)
        target_frequency = self.fft(batch_y.float(), dim=1)
        # fft shape: [B, P, D]
        if self.configs.auxi_type == 'complex':
            loss_auxi = frequency_outputs - target_frequency
        elif self.configs.auxi_type == 'complex-phase':
            loss_auxi = (frequency_outputs - target_frequency).angle()
        elif self.configs.auxi_type == 'complex-mag-phase':
            loss_auxi_mag = (frequency_outputs - target_frequency).abs()
            loss_auxi_phase = (frequency_outputs - target_frequency).angle()
            loss_auxi = torch.stack([loss_auxi_mag, loss_auxi_phase])
        elif self.configs.auxi_type == 'phase':
            loss_auxi = frequency_outputs.angle() - target_frequency.angle()
        elif self.configs.auxi_type == 'mag':
            loss_auxi = frequency_outputs.abs() - target_frequency.abs()
        elif self.configs.auxi_type == 'mag-phase':
            loss_auxi_mag = frequency_outputs.abs() - target_frequency.abs()
            loss_auxi_phase = frequency_outputs.angle() - target_frequency.angle()
            loss_auxi = torch.stack([loss_auxi_mag, loss_auxi_phase])
        else:
            raise NotImplementedError
