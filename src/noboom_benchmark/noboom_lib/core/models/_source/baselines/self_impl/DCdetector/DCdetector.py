...
from noboom_benchmark.noboom_lib.core.models._source.baselines.self_impl.DCdetector.DCdetector_model import DCdetector_model
...
class EarlyStopping:
    def __init__(self, patience=7, verbose=False, dataset_name="", delta=0):
        ...
        # NumPy 2.x removed the `np.Inf` alias.
...
class DCdetector:
    def __init__(self, **kwargs):
        ...
        for ps in self.config.patch_size:
            if self.win_size % ps != 0:
                raise ValueError(
                    f"DCdetector: win_size={self.win_size} is not divisible by "
    ...
    def detect_fit(self, train_data: pd.DataFrame, test_data: pd.DataFrame):
        ...
        for epoch in range(self.config.num_epochs):
            ...
            vali_loss = self.vali(self.valid_loader)
