...
from noboom_benchmark.noboom_lib.core.models._source.baselines.catch.layers.RevIN import RevIN
from noboom_benchmark.noboom_lib.core.models._source.baselines.catch.layers.cross_channel_Transformer import Trans_C
# Cell
import torch
...
from noboom_benchmark.noboom_lib.core.models._source.baselines.catch.layers.channel_mask import channel_mask_generator


@_dynamo_disable
def _ifft_from_real_imag(real: torch.Tensor, imag: torch.Tensor):
    complex_z = torch.complex(real.contiguous(), imag.contiguous())
    return torch.fft.ifft(complex_z), complex_z
...
class CATCHModel(nn.Module):
    ...
    def forward(self, z):  # z: [bs x seq_len x n_vars]
        ...
        z1 = self.head_f1(z1).float().contiguous()  # z: [bs x nvars x seq_len]
        z2 = self.head_f2(z2).float().contiguous()  # z: [bs x nvars x seq_len]

        z, complex_z = _ifft_from_real_imag(z1, z2)
        zr = z.real
        zi = z.imag
